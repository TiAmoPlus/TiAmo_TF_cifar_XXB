...
# 加载测试数据
with open(file_path + "test_batch", mode='rb') as file:
...
# y = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2, name="final_result")
y = tf.nn.softmax(tf.matmul(h_fc1, W_fc2) + b_fc2, name="final_result")
...
for i in range(20000):
    ...
    train_step.run(feed_dict={x: x_train[start: start + BATCH_SIZE], y_: y_train[start: start + BATCH_SIZE], keep_prob: 0.5})
